...
describe("manifest.json", () => {
  ...
  it("both tables are adult_writable (leaders record, everyone views)", () => {
    expect(manifest.row_policies?.equipment?.kind).toBe("adult_writable");
    expect(manifest.row_policies?.checkouts?.kind).toBe("adult_writable");
  });

  it("due_date is declared plaintext for the overdue export", () => {
    expect(manifest.db_plaintext_columns).toContain("due_date");
  });

  it("ai exports match the query files", () => {
    expect(manifest.ai_access?.db_exports?.sort()).toEqual(["equipment", "open_checkouts"]);
  });
});
